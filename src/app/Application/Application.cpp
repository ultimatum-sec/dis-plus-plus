...
void Application::Disassemble(const std::filesystem::path &path) noexcept(false)
{
	...
	auto *pEditor{static_cast<disxx::ui::SourceEditor *>(s_pInstance->m_pWindow.get()->GetWidgets().at(0).get())};
	...
	pEditor->AddLine(";{:*<64}", "");
   	...
   	pEditor->AddLine(";  Dis++ version: " __DISXX_VERSION__ " - {}", path.string());
   	pEditor->AddLine(";{:*<64}", "");
   	pEditor->AddLine("");
   	pEditor->AddLine(";{:=<63};", "");
	pEditor->AddLine("");
	...
	pEditor->Resize(disxx::ui::utility::Vec2<float>{width * 1.f, height * 0.7f});
